...
import { DetailedError } from "./appUtils";
...
function useAppLoginForm(params?: AppLoginFormParams): AppLoginFormModel {
    const struct: AppLoginFormStruct = {
        ...
        mount: async () => {
            await model.bus.unicast("Alert.Information", { message: 'Use admin/admin credentials to log in.' });
        },
    ...
    async function _basicLogin() {
        try {
            ...
            if (e instanceof DetailedError) {
                await model.dialogError(e.name, e.message, e.details);
                return;
            }
